...
import { FileWarning, Search, ChevronDown, ChevronUp, User } from 'lucide-react';
...
function DoutorBlock({ group, query }: { group: DoutorGroup; query: string }) {
  ...
  return (
    <div className="rounded-xl border border-burst-border bg-black/20 overflow-hidden">
      ...
      >
        <div className="w-8 h-8 rounded-lg bg-burst-warning/15 text-burst-warning flex items-center justify-center shrink-0">
          <User size={15} />
        </div>
        <span className="font-display text-base tracking-wide text-white">{group.doutor}</span>
        <span className="ml-2 text-xs px-2 py-0.5 rounded bg-burst-warning/15 text-burst-warning font-bold uppercase tracking-wider">
          Chat incompleto
        </span>
        <span className="text-xs px-2 py-0.5 rounded-full bg-white/5 text-burst-muted">
          {group.totalChats} lead{group.totalChats !== 1 ? 's' : ''}
        </span>
          ...
          <table className="w-full text-sm">
            <thead className="bg-black/30 text-[10px] uppercase tracking-widest text-burst-muted">
              <tr>
                ...
                <th className="text-center px-4 py-2 font-semibold">Cadastros</th>
                <th className="text-left px-4 py-2 font-semibold">Último</th>
              </tr>
            </thead>
            <tbody>
              {clientes.map((c) => (
                <tr key={c.telefone} className="border-t border-burst-border/50 hover:bg-white/[0.02]">
                  ...
                  <td className="px-4 py-2 text-center">
                    <span className="inline-flex items-center justify-center w-7 h-7 rounded-full font-display text-sm font-bold bg-burst-warning/15 text-burst-warning">
                      {c.quantidade}
                    </span>
                  </td>
...
export function ChatsIncompletos({ leads }: Props) {
  ...
  return (
    <section className="rounded-2xl bg-burst-card border border-burst-warning/40 p-6">
      <div className="flex items-center gap-2 mb-4">
        <FileWarning className="text-burst-warning" size={20} />
        <h3 className="font-display text-xl tracking-wider text-white">
          Chats Incompletos
        </h3>
        <span className="ml-2 text-xs px-2 py-0.5 rounded-full bg-burst-warning/15 text-burst-warning font-bold">
      ...
      <p className="text-xs text-burst-muted mb-3 -mt-2">
        Esses doutores estão armazenados mas <strong className="text-burst-warning">não contam</strong> na taxa de transferência nem no CPT.
      </p>
